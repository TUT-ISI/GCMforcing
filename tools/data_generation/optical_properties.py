...
Reads cross section of particles from look-up file based on given data
"""

def cross_section(pxx,pnr,pni,lookup):
    ...
    # sigma_n = extinction cross section in table n
    sigma_1='sigma_1'
    sigma_2='sigma_2'

    # read 3D sigma_n from the lut
    lut1=(np.array(lookup.variables['sigma_1']))
    lut2=(np.array(lookup.variables['sigma_2']))

    # retrieve the sigma values corresponding to pxx, pnr, and pni 
    sigma=lut1[Ndis,Nni,Nnr]*(1-ktable)+lut2[Ndis,Nni,Nnr]*ktable 

    lut1=(np.array(lookup.variables['omega_1']))
    lut2=(np.array(lookup.variables['omega_2']))

    omega=lut1[Ndis,Nni,Nnr]*(1-ktable)+lut2[Ndis,Nni,Nnr]*ktable 
