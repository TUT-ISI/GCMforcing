...
Calculates cloud droplet number consentration and aerosol cloud radiative effect 
...
# program for calculating CDNC and energy balance with aerosol cloud radiative effect

# define the function for operating this code outside this file
def running(model,other_model,intime,light,mmrsel,nmrsel,updraftsel,cltsel,clwsel,avoid_save_on,calculate_CRE,paths):
    ...
    output=j(out,f"{mmrn}_{nmrn}_{updn}_{cltn}_{clwn}/"+f"{other_model}_{mmrn}_{nmrn}_{updn}_{cltn}_{clwn}_out/")
    ...
    tf=j(tp,f"temp_{mmrn}_{nmrn}_{updn}_{cltn}_{clwn}/"+f"temp_{other_model}_{intime}_{mmrn}_{nmrn}_{updn}_{cltn}_{clwn}/")
    ...
    fname5=j(output,'cdnc_2010'+intime+'_'+dim+f"{mmrn}_{nmrn}_{updn}_{cltn}_{clwn}_for_{other_model}"+'.nc')
    fname6=j(output,'CRE_2010'+intime+'_'+dim+f"{mmrn}_{nmrn}_{updn}_{cltn}_{clwn}_for_{other_model}"+'.nc')

    # check if this iteration exists
    if avoid_save_on:
        if calculate_CRE:
            if check_similiar(fname6):
                # if the iteration is allready completed, skip to the next one
                print("Iteration skipped due to dublicate on file "+fname6)
                return
        else:
            if check_similiar(fname5):
                # if the iteration is allready completed, skip to the next one
    ...
    nmr_temp = j(tf,intime+'nmr_temp'+other_model+'.nc')
    ...
    fname_mmrham = j(ap,fname_mmrham)
    ...
    fname_mmrcomp = j(ap,fname_mmrcomp)
        # select month for MMR comparison data
    cdo_copy(fname_mmrcomp, compfile, selmon)
    fname_mmrcomp = compfile

    # file for NMR
    fname_nmr='aerocom3_'+other_model+'-met2010_AP3-CTRL_nmr_ModelLevel_2010_monthly_T63L47.nc'
    fname_nmr=j(paths[3],fname_nmr) # Find correct path to file
    # Select time for NMR data
    cdo_copy(fname_nmr, nmr_temp, selmon)
    fname_nmr = nmr_temp
    ...
    if mmrsel=='mmr' or nmrsel=='nmr':
        znaero,zvols = read_aero_binsdp(tracerfile, vphyscfile, ratio, mmrsel, nmrsel, fname_nmr)
    else:
    ...
    if updraftsel=='updraft':
        ...
        fname_updraft=j(ap,fname_updraft)
    ...
    psmax = activdata.variables['SWAT_MAX_STRAT']
    ...
    if calculate_CRE:
        ...
        if clwsel=='clw':
            ...
            # read liquid water content data from SALSA data
        ...
        # calculate the aerosol cloud radiative effect
        print("Calculating energy balance with cloud radiative effect")
        ...
        print(f"CRE dimensions are {E.shape}")
        ...
        if os.path.exists(fname6):  
            os.remove(fname6)   # if the file exists already, delete it   
        write_3D_grid(fname6,E,lon,lat,time,'CRE')
