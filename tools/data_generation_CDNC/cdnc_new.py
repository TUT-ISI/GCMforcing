...
def cloud_activation(pnaero, pvols, ptm1, papm1, pqm1, pw, nw, psmax, psmax_fixed):
    ...
    for b in bins:
        ...
        if b[0]!='1':
            ...
            znshi = zns[b]*pnaero[numkey]*vhilim[bin_index]/zvols_tot[b]
            
            #-- lower bound of critical supersaturation
            zcc = np.sqrt(3.*zbb*znshi/zaa)
            zdd = (6./np.pi)*vhilim[bin_index]*zvols_insol[b]/zvols_tot[b]
            tmp = lnS_crit_insol_over_A(zcc, zdd)
            sil = np.exp(zaa*tmp)-1.

            #-- moles of solute at the lower bound of the bin:
            znslo = zns[b]*pnaero[numkey]*vlolim[bin_index]/zvols_tot[b]
            
            #-- upper bound of critical supersaturation
            zcc = np.sqrt(3.*zbb*znslo/zaa)
            zdd = (6./np.pi)*vlolim[bin_index]*zvols_insol[b]/zvols_tot[b]
            tmp = lnS_crit_insol_over_A(zcc, zdd)
